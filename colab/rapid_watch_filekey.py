...
def main():
    ...
    # Free-tier 16 GB GPU benchmark: 4 workers gave the best stable throughput.
    # 6 workers were slower and 8 workers caused ONNXRuntime CUDA OOM on large pages.
    if '--workers' in rest:
        i=rest.index('--workers')
        try: rest[i+1]=str(min(4,max(1,int(rest[i+1]))))
        except Exception: rest[i+1]='4'
    else:
        rest.extend(['--workers','4'])
    ...
    print('RAPID_FILEKEY_CONFIG workers=4',flush=True)
