...
def main():
    ...
    print(f'RAPID_WATCH_READY poll={a.poll}s workers={a.workers} downloaders={a.downloaders} claim={a.claim}',flush=True)
    while True:
        ...
        try:
            ...
            sftp.close(); tr.close()
        print(f'RAPID_WATCH_POLL tick={tick} uptime={time.time()-started:.0f}s rows={len(rows)} groups={len(groups)} missing={missing} complete={len(rows)-missing}/{len(rows)} claim_sha={h[:10]} poll_sec={time.time()-poll_t0:.2f}',flush=True)
        if rows and missing:
