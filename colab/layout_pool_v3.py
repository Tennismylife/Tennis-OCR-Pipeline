...
def main():
    ...
    ap.add_argument('--generation-label',default='colab_layout_v3'); ap.add_argument('--workdir',default='/content/tml_layout_pool_v3'); a=ap.parse_args()
    ...
    print(f'LAYOUT_POOL_START workers={a.workers} downloaders={a.downloaders} poll={a.poll}s status_every={a.status_every}s generation={a.generation_label} device={a.device.upper()}',flush=True)
    ...
    try:
        while True:
            ...
            print(f'LAYOUT_CLAIM_START stage={stage} rows={len(rows)} todo={len(todo)} workers={a.workers} device={a.device.upper()}',flush=True); last_stage=stage
            ...
            while pending:
                ready,_=wait(pending,timeout=a.status_every,return_when=FIRST_COMPLETED)
                now=time.time(); elapsed=max(.001,now-t0); ppm=done*60/elapsed; remaining=max(0,total-done); eta=remaining/ppm if ppm>0 else -1; u,m,p=device_snapshot(a.device)
                if not ready:
                    print(f'LAYOUT_HEARTBEAT stage={stage} completed={done}/{total} uploaded={uploaded} inflight={len(pending)} errors={err} ppm={ppm:.2f} eta_min={eta:.2f} last_done_age={now-last_done_at:.1f}s device={a.device.upper()} gpu={u:.0f}% vram={m:.0f}MiB power={p:.0f}W',flush=True); continue
                for f in ready:
                    idx_ctx,r_ctx=pending.pop(f)
                    try:
                        ...
                        print(f'LAYOUT_DONE progress={done}/{total} idx={idx}/{tot} stem={stem} sec={sec:.2f} rows={nrows} blocks={blocks} uploaded={uploaded} inflight={len(pending)} device={a.device.upper()} gpu={u:.0f}% vram={m:.0f}MiB power={p:.0f}W',flush=True)
                    except Exception as e:
                ...
                if now-last_status>=a.status_every:
                    ...
                    print(f'LAYOUT_HEARTBEAT stage={stage} completed={done}/{total} uploaded={uploaded} inflight={len(pending)} errors={err} ppm={ppm:.2f} eta_min={eta:.2f} last_done_age={now-last_done_at:.1f}s device={a.device.upper()} gpu={u:.0f}% vram={m:.0f}MiB power={p:.0f}W',flush=True); last_status=now
            elapsed=max(.001,time.time()-t0); print(f'LAYOUT_BATCH_COMPLETE stage={stage} completed={done}/{total} uploaded={uploaded} errors={err} elapsed={elapsed:.1f}s ppm={done*60/elapsed:.2f} device={a.device.upper()}',flush=True)
