...
def init_gpu(layout_py,config_json,workers):
    ...
    os.environ.setdefault('PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK','True'); os.environ.setdefault('FLAGS_allocator_strategy','auto_growth')
...
def layout_one(item):
    ...
    payload.update(source_image=row['source_image'],source_ocr_json=row['ocr_json'],compute_device='CUDA',layout_generation='colab_a100_layout_v1',colab_workers=WORKERS)
...
def main():
    ...
    sftp.get('/home/andre/GallicaJobs/_shared/layout_ocr/layout_map.py',str(runtime/'layout_map.py')); sftp.get('/home/andre/GallicaJobs/_shared/layout_ocr/config.json',str(runtime/'config.json')); sftp.close(); tr.close()
    print(f'LAYOUT_POOL_START workers={a.workers} downloaders={a.downloaders} watch=1',flush=True)
    ctx=mp.get_context('spawn'); pool=ProcessPoolExecutor(max_workers=a.workers,mp_context=ctx,initializer=init_gpu,initargs=(str(runtime/'layout_map.py'),str(runtime/'config.json'),a.workers)); last_stage=None
