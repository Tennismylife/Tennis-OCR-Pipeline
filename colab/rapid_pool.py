...
ENGINE=None; PROFILE='HQ'; GENERATION='colab_batch'; DEVICE='cuda'

def init_engine(profile,generation,device):
    global ENGINE,PROFILE,GENERATION,DEVICE
    PROFILE=profile; GENERATION=generation; DEVICE=device
    ENGINE=build_engine(profile,device)
    print(f'OCR_ENGINE_READY pid={os.getpid()} profile={PROFILE} generation={GENERATION} device={DEVICE.upper()}',flush=True)

def ocr_one(item):
    idx,total,r,ip,outdir=item
    ark=(r.get('ark') or '').strip(); page=pg(r.get('page')); stem=f'{ark}_f{page}'; t0=time.time()
    print(f'OCR_START idx={idx}/{total} stem={stem} ark={ark} page={page} pid={os.getpid()} device={DEVICE.upper()}',flush=True)
    rows=ocr_image(ip,ENGINE)
    dev='CUDA' if DEVICE=='cuda' else 'CPU'
    files=write_payload(outdir,ark,page,rows,PROFILE,(r.get('source_image') or 'COLAB'),dev)
    jp=Path(files[0]); d=json.loads(jp.read_text(encoding='utf-8'))
    d['ocr_generation']=GENERATION; d['gpu_pool']=(DEVICE=='cuda'); d['cpu_pool']=(DEVICE=='cpu')
...
def main():
    ...
    ap.add_argument('--device',choices=['cuda','cpu'],default='cuda'); ap.add_argument('--status-every',type=int,default=10)
    ...
    if a.autotune and not a.no_autotune and a.device=='cuda':
    ...
    def upload(idx,stem,files):
        ...
        for attempt in (1,2):
            try:
                ...
                if attempt==2: raise
    with ThreadPoolExecutor(max_workers=a.downloaders) as dl, ProcessPoolExecutor(max_workers=workers,mp_context=ctx,initializer=init_engine,initargs=(a.profile,generation,a.device)) as pool:
        fetch_pending={dl.submit(fetch,idx,r):(idx,r) for idx,r in enumerate(rows,1)}; ocr_pending={}
        while fetch_pending or ocr_pending:
            now=time.time()
            if now-last_status>=a.status_every:
                u,m,p=device_snapshot(a.device); completed=done+cached; elapsed=max(.001,now-t0); ppm=done*60/elapsed; remaining=max(0,total-completed); eta=(remaining/ppm) if ppm>0 else -1
                print(f'RAPID_HEARTBEAT elapsed={elapsed:.1f}s completed={completed}/{total} ocr_done={done} cached={cached} uploaded={uploaded} fetched={fetched} inflight={len(ocr_pending)} fetch_pending={len(fetch_pending)} errors={err} ppm={ppm:.2f} eta_min={eta:.2f} last_done_age={now-last_done_at:.1f}s device={a.device.upper()} gpu={u:.0f}% vram={m:.0f}MiB power={p:.0f}W',flush=True); last_status=now
            ready_ocr={f for f in ocr_pending if f.done()}
            for f in ready_ocr:
                idx_ctx,stem_ctx=ocr_pending.pop(f)
                try:
                    idx,tot,stem,files,sec,nrows=f.result(); upload(idx,stem,files); done+=1; last_done_at=time.time(); completed=done+cached
                    print(f'OCR_DONE progress={completed}/{total} idx={idx}/{tot} stem={stem} sec={sec:.2f} rows={nrows} inflight={len(ocr_pending)} device={a.device.upper()}',flush=True)
                except Exception as e:
            ...
            if fetch_pending:
                ...
                for f in ready_fetch:
                    ...
                    try:
                        ...
                        fetched+=1
                        fut=pool.submit(ocr_one,(idx,total,r,ip,str(outdir))); ocr_pending[fut]=(idx,stem)
                        print(f'OCR_QUEUED idx={idx}/{total} stem={stem} inflight={len(ocr_pending)}',flush=True)
                    except Exception as e:
                        err+=1; print(f'FETCH_ERROR idx={idx_ctx}/{total} stem={stem_ctx} {type(e).__name__}: {e}',flush=True)
            elif ocr_pending and not ready_ocr: wait(ocr_pending,timeout=.20,return_when=FIRST_COMPLETED)
    try:
        if upload_sftp: upload_sftp.close()
        if upload_tr: upload_tr.close()
    except Exception: pass
    elapsed=max(.001,time.time()-t0); u,m,p=device_snapshot(a.device); print(f'RAPID_FINAL completed={done+cached}/{total} ocr_done={done} cached={cached} uploaded={uploaded} errors={err} elapsed={elapsed:.1f}s ppm={done*60/elapsed:.2f} device={a.device.upper()} gpu={u:.0f}% vram={m:.0f}MiB power={p:.0f}W',flush=True)
